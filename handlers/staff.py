...
Navigation:
  ...
  🏠 / 🔄   — ReplyKeyboard fallbacks, exit any active conversation
"""
...
async def nav_fallback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 🔄 Restart clears temporary data; 🏠 or /cancel just cancels
    if "🔄" in text:
        for key in ("new_plate", "new_service", "new_pay_type", "upd_wf_id", "upd_pm_id"):
            ctx.user_data.pop(key, None)
        reply = t("restarted", lang)
    else:
        reply = t("cancelled", lang)
